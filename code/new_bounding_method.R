...
###########################################
## Get 2011-2012 hourly feter level data ##
###########################################
setwd("/Users/yutian/Google Drive/global_trends/data/Demand_Data/Original/DELHI_08-13/big_data/11-12/")
options(stringsAsFactors=FALSE)

### list of folder paths
filename <- list.files()
filepath1 <- filename[1:52]

filepath2 <- lapply(filepath1, function(x) grep("Discom",list.files(x),value=TRUE))
tmp <- unlist(lapply(filepath2, function(x) length(x)>1))
filepath2[8] <- filepath2[8][[1]][1]
filepath2[32] <- filepath2[32][[1]][1]
filepath2[34] <- filepath2[34][[1]][1]
filepath2[36] <- filepath2[36][[1]][1]
filepath2[49] <- filepath2[49][[1]][1]
rm(tmp)
filepath2 <- unlist(filepath2)
